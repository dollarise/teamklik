...
async function load() {
  ...
  root.innerHTML = `<main class="landing"><section class="landing-card offer-card"><div class="member-name">${esc(member.name || "Member")}</div>${bannerHtml}<div class="offer-copy"><h2>${esc(member.campaign_name || "Exclusive Offer")}</h2><p>Take the next step and explore this opportunity.</p>${member.offer_url ? `<a class="landing-button" href="${esc(member.offer_url)}" target="_blank" rel="noopener noreferrer">UNLOCK EXCLUSIVE ACCESS</a>` : ""}</div></section><p class="footer-note">Work Smarter. Earn More Online</p></main>`;
