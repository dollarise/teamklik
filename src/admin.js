...
function toast(msg) {
  ...
  clearTimeout(window.__t); window.__t = setTimeout(() => el.classList.remove("show"), 2200);
}
...
function render() {
  ...
  app.innerHTML = `
    <main class="admin-page">
      ...
      <section class="summary">
        <div><span>USERS</span><strong>${num(state.users.length)}</strong></div>
        <div><span>CAMPAIGNS</span><strong>${num(state.campaigns.length)}</strong></div>
        <div><span>IMPRESSIONS</span><strong>${num(totalImpressions)}</strong></div>
        <div><span>CLICKS</span><strong>${num(totalClicks)}</strong></div>
      ...
      <section class="panel">
        ...
        <div class="table-wrap">
          <table>
            <thead><tr><th>Name</th><th>Membership</th><th>Status</th><th>Role</th><th>Actions</th></tr></thead>
            <tbody>
              ${state.users.map(u => `
                <tr>
                  <td><strong>${esc(u.name)}</strong><small>${esc(u.id)}</small></td>
                  <td>${esc(u.membership)}</td>
                  <td><span class="badge">${esc(u.account_status)}</span></td>
                  <td>${esc(u.role)}</td>
      ...
      <section class="panel">
        ...
        <div class="table-wrap">
          <table>
            <thead><tr><th>Campaign</th><th>User</th><th>Impressions</th><th>Clicks</th><th>CTR</th><th>CPM</th><th>Revenue</th><th>Actions</th></tr></thead>
            <tbody>
              ${state.campaigns.map(c => {
                const u = state.users.find(x => x.id === c.user_id);
                return `<tr>
                  <td><strong>${esc(c.name)}</strong></td>
                  <td>${esc(u?.name || c.user_id)}</td>
                  ...
                  <td>
                    <button class="edit-campaign" data-id="${c.id}">EDIT</button>
                    <button class="delete-campaign danger" data-id="${c.id}">DELETE</button>
                  </td>
                </tr>`;
              }).join("")}
            </tbody>
  ...
  document.querySelectorAll(".edit-campaign").forEach(btn => {
    btn.onclick = () => campaignForm(state.campaigns.find(c => c.id === Number(btn.dataset.id)));
  });

  document.querySelectorAll(".delete-campaign").forEach(btn => {
    btn.onclick = async () => {
      if (!confirm("Delete this campaign?")) return;
      const { error } = await supabase.from("campaigns").delete().eq("id", Number(btn.dataset.id));
      if (error) return toast(error.message);
      await loadData(); render(); toast("Campaign deleted.");
    };
  });

  document.querySelectorAll(".edit-user").forEach(btn => {
    btn.onclick = () => userForm(state.users.find(u => u.id === btn.dataset.id));
  });
...
function campaignForm(c = null) {
  ...
  app.innerHTML += `
    <div class="modal" id="modal">
      <form class="modal-card" id="campaignForm">
        <h2>${editing ? "Edit Campaign" : "New Campaign"}</h2>
        <label>User</label><select id="cUser" required>${options}</select>
        <label>Name</label><input id="cName" value="${esc(c?.name || "")}" required>
        <label>Impressions</label><input id="cImp" type="number" min="0" value="${c?.impressions ?? 0}" required>
        <label>Clicks</label><input id="cClicks" type="number" min="0" value="${c?.clicks ?? 0}" required>
        <label>CPM</label><input id="cCpm" type="number" min="0" step="0.01" value="${c?.cpm ?? 0}" required>
        <label>Revenue</label><input id="cRevenue" type="number" min="0" step="0.01" value="${c?.revenue ?? 0}" required>
        <label class="check"><input id="cActive" type="checkbox" ${c?.active !== false ? "checked" : ""}> Active</label>
        <div class="modal-actions"><button type="button" id="cancel">CANCEL</button><button class="primary" type="submit">SAVE</button></div>
      </form>
    </div>`;
  ...
  document.querySelector("#campaignForm").onsubmit = async (e) => {
    e.preventDefault();
    const payload = {
      user_id: document.querySelector("#cUser").value,
      name: document.querySelector("#cName").value.trim(),
      impressions: Number(document.querySelector("#cImp").value),
      clicks: Number(document.querySelector("#cClicks").value),
      cpm: Number(document.querySelector("#cCpm").value),
      revenue: Number(document.querySelector("#cRevenue").value),
    ...
    document.querySelector("#modal").remove();
    await loadData(); render(); toast("Campaign saved.");
  };
}
...
function userForm(u) {
  app.innerHTML += `
    <div class="modal" id="modal">
      <form class="modal-card" id="userForm">
        <h2>Edit User</h2>
        <label>Name</label><input id="uName" value="${esc(u.name)}" required>
        <label>Membership</label>
        <select id="uMembership">
          ${["Standard","Premium","VIP"].map(x => `<option ${u.membership === x ? "selected" : ""}>${x}</option>`).join("")}
        </select>
        <label>Status</label>
        <select id="uStatus">
          ${["Verified","Pending","Suspended"].map(x => `<option ${u.account_status === x ? "selected" : ""}>${x}</option>`).join("")}
        </select>
        <label>Role</label>
        <select id="uRole">
          ${["user","admin"].map(x => `<option ${u.role === x ? "selected" : ""}>${x}</option>`).join("")}
        </select>
        <div class="modal-actions"><button type="button" id="cancel">CANCEL</button><button class="primary" type="submit">SAVE</button></div>
      </form>
    </div>`;
  ...
  document.querySelector("#userForm").onsubmit = async (e) => {
    e.preventDefault();
    const payload = {
      name: document.querySelector("#uName").value.trim(),
      membership: document.querySelector("#uMembership").value,
      account_status: document.querySelector("#uStatus").value,
      role: document.querySelector("#uRole").value
    };
    const { error } = await supabase.from("profiles").update(payload).eq("id", u.id);
    if (error) return toast(error.message);
    document.querySelector("#modal").remove();
    await loadData(); render(); toast("User updated.");
  };
}
...
async function start() {
  try {
    ...
    app.innerHTML = `<main class="login-shell"><section class="login-card">
      <h1>Admin Access</h1><p>${esc(e.message)}</p>
      <button class="primary" id="back">BACK TO LOGIN</button>
    </section></main>`;
